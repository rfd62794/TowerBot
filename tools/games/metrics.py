...
class GameTools(BaseTool):
    """Game metrics tools with BaseTool pattern."""

    def get_game_metrics(self, game_name: str) -> dict:
        """
        Get detailed metrics for a specific game by name.
        ...
        resolved = resolve_appid(game_name)
        if not resolved:
            return self.error(f"Game not found: {game_name}", code="not_found")
        ...
        return self.success(result, stale_result=library_result)
    ...
    def get_installed_games(self) -> dict:
        ...
        library_result = get_game_library()
        if "error" in library_result:
            return self.error("Steam unavailable", code="api_failed")
    ...
    def get_sale_info(self, game_names: list[str]) -> dict:
        """
        Check current sale prices and historical lows for games via IsThereAnyDeal.
            ...
            if "error" in prices_result:
                for game_name in results:
                    if "error" not in results[game_name]:
                        results[game_name] = {"error": prices_result["error"]}
                result = {"games": results}
                return self.error("ITAD price lookup failed", code="api_failed")
...
# Backwards compat
def get_game_metrics(game_name: str) -> dict:
    return _games.get_game_metrics(game_name)


def get_installed_games() -> dict:
    return _games.get_installed_games()
